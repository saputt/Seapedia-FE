...
interface AppReviewWithName extends AppReview {
  reviewerName: string;
}
...
const StarInput = ({ value, onChange }: StarInputProps) => {
  return (
...
const TestimonialSection = () => {
  ...
  const reviews: AppReviewWithName[] = (data as any)?.reviews || [];

  const [form, setForm] = useState<ReviewFormState>({ reviewerName: "", rating: 0, comment: "" });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    submitReview.mutate(
      { reviewerName: form.reviewerName, rating: form.rating, comment: form.comment } as any,
      {
        onSuccess: () => {
          ...
          <div className="lg:col-span-3">
            ...
            {!isLoading && !isError && reviews.length > 0 && (
              <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                {reviews.map((review: AppReviewWithName) => (
