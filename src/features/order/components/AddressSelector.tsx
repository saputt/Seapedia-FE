...
const AddressSelector = ({ isOpen, onClose, onSelect, selectedId }: AddressSelectorProps) => {
  const queryClient = useQueryClient();
  const { data: addresses = [], isLoading } = useAddresses(isOpen);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<AddressFormState>({ label: "", completeAddress: "" });
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      await createAddress(form as any);
                ...
                addresses.map((addr: Address) => (
                  ...
                  >
                    ...
                    <label htmlFor={`addr-modal-${addr.id}`} className="flex-1 cursor-pointer">
                      ...
                      {(addr as Address & { completeAddress?: string }).completeAddress && (
