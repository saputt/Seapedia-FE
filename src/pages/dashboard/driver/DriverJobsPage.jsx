...
const DriverJobsPage = () => {
  const { data: myJobs = [] } = useMyDriverJobs();
  const { data: availableJobs = [], isLoading: availableLoading, error: availableError } = useAvailableJobs();
  const takeJobMutation = useTakeJob();
  const deliveryDoneMutation = useDeliveryDone();

  const activeJobs = myJobs.filter(
    (job) => job.status === "ON_DELIVERY" && !job.driverJob?.doneAt
  );
  ...
  const renderJobCard = (job, showTakeButton) => (
